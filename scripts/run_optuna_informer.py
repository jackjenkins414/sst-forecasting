"""
Optuna hyperparameter search for Isaac's ProbSparseInformer.

Each trial saves results under experiments/results/ in the same format as
all other HPO scripts so compare_runs.py can read them together.
Study is persisted to experiments/optuna_informer.db.

Usage
-----
    python scripts/run_optuna_informer.py              # 20 trials (default)
    python scripts/run_optuna_informer.py --n_trials 30
    python scripts/run_optuna_informer.py --show       # print study summary only
"""
...
DB_PATH     = PROJECT_ROOT / "experiments/optuna_informer.db"

CONTEXT_LEN         = 90
HORIZON             = 7
BATCH_SIZE          = 8   # encoder distillation + decoder cross-attn is memory-heavy
NUM_EPOCHS          = 50
...
# Search space
#
# factor    - ProbSparse sparsity factor (controls how many top-k queries
#             are kept per attention computation; lower = more sparse)
# label_len - decoder start-token length (overlap with encoder input)

SEARCH_SPACE = {
    "lr":               FloatDistribution(1e-4, 2e-3, log=True),
    "d_model":          CategoricalDistribution([64, 128, 256]),
    "n_heads":          CategoricalDistribution([4, 8]),
    "n_encoder_layers": IntDistribution(2, 4),
    "n_decoder_layers": IntDistribution(1, 2),
    "d_ff_mult":        CategoricalDistribution([2, 4]),   # d_ff = d_model * mult
    "dropout":          FloatDistribution(0.05, 0.35),
    "factor":           CategoricalDistribution([3, 5, 7]),
    "label_len":        CategoricalDistribution([14, 28, 45]),
    "lr_factor":        FloatDistribution(0.4, 0.8),
    "anomaly_alpha":    FloatDistribution(0.0, 0.20),
}

# Known-good baseline (Isaac's defaults) - injected as trial 0.
...
# Seed study with previous Informer runs
...
def _params_from_config(config: dict) -> dict | None:
    ...
    mapping = {
        ...
        "n_encoder_layers": config.get("n_encoder_layers"),
        "n_decoder_layers": config.get("n_decoder_layers"),
...
def make_objective(land_mask_np, norm_mean, norm_std,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        n_encoder_layers = trial.suggest_int("n_encoder_layers",   SEARCH_SPACE["n_encoder_layers"].low,
                                                                    SEARCH_SPACE["n_encoder_layers"].high)
        n_decoder_layers = trial.suggest_int("n_decoder_layers",   SEARCH_SPACE["n_decoder_layers"].low,
                                                                    SEARCH_SPACE["n_decoder_layers"].high)
        d_ff_mult        = trial.suggest_categorical("d_ff_mult",  SEARCH_SPACE["d_ff_mult"].choices)
        dropout          = trial.suggest_float("dropout",          SEARCH_SPACE["dropout"].low,
                                                                    SEARCH_SPACE["dropout"].high)
        factor           = trial.suggest_categorical("factor",     SEARCH_SPACE["factor"].choices)
        label_len        = trial.suggest_categorical("label_len",  SEARCH_SPACE["label_len"].choices)
        ...
        model = ProbSparseInformer(
            ...
            factor=factor, label_len=label_len,
        ).to(device)
...
def main():
    ...
    if not any(t.params == SEED_CONFIG for t in study.trials):
        ...
        print("Queued Isaac's baseline config as trial 0")

    n_loaded = load_previous_runs(study)
    print(f"Loaded {n_loaded} previous Informer runs into study "
    ...
    _print_summary(study)
    save_study_plots(study, model_name="informer")
