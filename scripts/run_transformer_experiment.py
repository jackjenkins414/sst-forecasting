...
from src.models.transformer import SstFlatTransformer
...
def main():
    ...
    transformer_rmse_per_step = rmse_per_step(
        test_preds_celsius, test_targets_celsius, land_mask=land_mask_np,
    )
    transformer_rmse_mean = float(transformer_rmse_per_step.mean())
    transformer_mae = mae(test_preds_celsius, test_targets_celsius, land_mask=land_mask_np)
    ...
    # Skill scores
    rmse_skill = skill_score(transformer_rmse_mean, persistence_rmse_mean)
    mae_skill = skill_score(transformer_mae, persistence_mae)
    ...
    print("\nTransformer (test, °C):")
    for h, r in enumerate(transformer_rmse_per_step, start=1):
        print(f"  RMSE day {h}: {r:.4f}")
    print(f"  RMSE mean:  {transformer_rmse_mean:.4f}")
    print(f"  MAE  mean:  {transformer_mae:.4f}")
