...
OUT_DIR   = PROJECT_ROOT / "experiments/ar_rollout_avg"

DEFAULT_MODELS = ["patch_transformer", "convlstm", "tubelet"]
HORIZON_STEP = 7  # native training horizon = block length per forward pass
COLOURS = {
...
def _chunked_rollout_phase(model, x: torch.Tensor, max_horizon: int, offset: int,
    ...
    """
    One 7-day-block-feedback rollout with chunk boundaries offset by `offset` days:
    a partial first block of length `offset` (skipped when offset == 0), then full
    7-day blocks. Each committed block feeds its own coherent frames back into the
    context. Returns (B, max_horizon, H, W) in normalised space.
    """
    ctx = x.to(device)
    chunks = []
    day = 0
    first = True
    while day < max_horizon:
        out = model(ctx)                                  # (B, 7, H, W): days day+1..day+7
        blk = offset if (first and offset > 0) else HORIZON_STEP
        blk = min(blk, max_horizon - day)
        chunks.append(out[:, :blk])                       # commit days day+1..day+blk
        # advance context by blk: drop oldest blk frames, append the blk committed frames
        ctx = torch.cat([ctx[:, blk:], out[:, :blk].unsqueeze(2)], dim=1)
        day += blk
        first = False
...
def autoregressive_predict_avg(model, x: torch.Tensor, max_horizon: int,
    ...
    Phase-averaged chunked rollout: average 7 block-feedback rollouts whose chunk
    boundaries are offset by 0..6 days. Each target day is committed exactly once
    per phase, so this is a uniform mean over the 7 phases.
    Returns (B, max_horizon, H, W) in normalised space.
    """
    B, _, _, Hg, Wg = x.shape
    acc = torch.zeros((B, max_horizon, Hg, Wg), device=device)
    with torch.no_grad():
        for offset in range(HORIZON_STEP):                # phases 0..6
...
def main():
    ...
    parser.add_argument("--max-horizon", type=int, default=49,
                        help="How many days to roll out (default 49 = 7 x 7-day blocks).")
    ...
    for mt in args.models:
        ...
        for bi, start in enumerate(range(0, n, args.batch_size), start=1):
            ...
            preds_norm = autoregressive_predict_avg(model, x_batch,
