"""
Optuna hyperparameter search for StackedSpatialLSTM.
...
Study is persisted to experiments/optuna_lstm.db.

Usage
-----
    python scripts/run_optuna_lstm.py              # 20 trials (default)
    python scripts/run_optuna_lstm.py --n_trials 50
    python scripts/run_optuna_lstm.py --show       # print study summary only
"""
...
DB_PATH     = PROJECT_ROOT / "experiments/optuna_lstm.db"
...
SEARCH_SPACE = {
    ...
    "hidden_size":   CategoricalDistribution([64, 128, 256]),
    "d_spatial":     CategoricalDistribution([32, 64]),       # 128 explodes encoder params
    "num_layers":    IntDistribution(1, 3),
    "dropout":       FloatDistribution(0.05, 0.40),
    "lr_factor":     FloatDistribution(0.4, 0.8),
    "anomaly_alpha": FloatDistribution(0.0, 0.20),
}

# Known-good baseline config to inject as trial 0.
...
def _params_from_config(config: dict) -> dict | None:
    ...
    mapping = {
        ...
        "d_spatial":     config.get("d_spatial"),
...
def make_objective(land_mask_np, norm_mean, norm_std,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        hidden_size   = trial.suggest_categorical("hidden_size", SEARCH_SPACE["hidden_size"].choices)
        d_spatial     = trial.suggest_categorical("d_spatial",   SEARCH_SPACE["d_spatial"].choices)
        num_layers    = trial.suggest_int("num_layers", SEARCH_SPACE["num_layers"].low,
        ...
        model = StackedSpatialLSTM(
            ...
            d_spatial=d_spatial, hidden_size=hidden_size,
...
def main():
    ...
    # Inject known-good baseline as trial 0 so TPE starts from a real anchor
    if not any(t.params == SEED_CONFIG for t in study.trials):
        study.enqueue_trial(SEED_CONFIG)
        print("Queued known-good baseline config as trial 0")

    n_loaded = load_previous_runs(study)
    print(f"Loaded {n_loaded} previous LSTM runs into study "
